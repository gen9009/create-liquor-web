...
var parseArgs_default = (0, import_node_util.parseArgs)({
  args,
  // 传递的参数数组
  options,
  // 定义的参数选项
...
async function init() {
  const { values: argv, positionals } = parseArgs_default;
  console.log("\u{1F680}::::::\u{1F436}\u{1F4A9}", argv);
  let answer = [];
  async function askNext() {
    answer = await (0, import_prompts.default)([
      {
        type: "text",
        name: "projectName",
        message: `\u{1F34B} \u8BF7\u8F93\u5165\u4F60\u5E0C\u671B\u521B\u5EFA\u7684\u9879\u76EE\u540D\u79F0?`,
        initial: "liquor-demo",
        validate: (value) => !isValidPackageName(value) ? "\u{1F352} \u9879\u76EE\u540D\u79F0\u4E0D\u5408\u6CD5" : true
      },
      {
        type: "confirm",
        name: "shouldOverwrite",
        message: "\u{1F34E} \u662F\u5426\u8986\u76D6\u540C\u540D\u9879\u76EE?"
      },
      {
        type: "select",
        name: "cssEngine",
        message: "\u{1F353} \u8BF7\u9009\u62E9\u4F60\u5E0C\u671B\u96C6\u6210\u7684 CSS\u9884\u8BBE?",
        choices: [
      ...
      },
      {
        type: "select",
        name: "ui",
        message: "\u{1F353} \u8BF7\u9009\u62E9\u4F60\u5E0C\u671B\u96C6\u6210\u7684 UI\u7EC4\u4EF6\u5E93?",
        choices: [
  ...
  try {
    let render = function(templateName) {
      const templateTarget = path2.resolve(__dirname, `template/${templateName}`);
      renderTemplate_default(templateTarget, dest);
    };
    console.log();
    console.log((0, import_picocolors.blue)((0, import_picocolors.bold)(`\u521B\u5EFA\u4F60\u7684 vue3+vite+ts+ui \u6A21\u7248\u{1F447}${(0, import_picocolors.yellow)("\u4FBF\u4E8E\u4E66\u5199demo")}`)));
    console.log();
    const answer2 = await askNext();
    console.log("\u{1F680}::::::\u{1F436}\u{1F4A9}", answer2);
    const { shouldOverwrite, projectName, cssEngine, ui } = answer2;
    let dest = path2.join(process.cwd(), projectName);
    if (fs2.existsSync(path2.join(process.cwd(), projectName))) {
      fs2.rmSync(path2.join(process.cwd(), projectName), { recursive: true });
    } else {
      fs2.mkdirSync(dest);
    }
    render("base");
    ...
    console.log(``);
    console.log((0, import_picocolors.bold)((0, import_picocolors.green)(`cd ${projectName}`)));
    console.log((0, import_picocolors.bold)((0, import_picocolors.green)(`${packageManager} install`)));
    console.log((0, import_picocolors.bold)((0, import_picocolors.green)(`${packageManager == "npm" ? "npm run dev" : `${packageManager} dev`}`)));
    console.log(``);
  } catch (error) {
    console.log(`\u{1F449}\u{1F3FC} Error: \u6784\u5EFA\u5931\u8D25---${JSON.stringify(error)}`);
  }
}
init();
